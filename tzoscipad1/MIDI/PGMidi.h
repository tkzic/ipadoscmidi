...
// send midi byte stream to specified endpoint
- (void) sendBytes:(const UInt8*)bytes size:(UInt32)size toEndpointName: (NSString *) name;
- (void) sendPacketList:(const MIDIPacketList *)packetList toEndpointName: (NSString *) name;

// tz exposed these for debugging

- (void) disconnectSource:(MIDIEndpointRef)endpoint;
- (void) disconnectDestination:(MIDIEndpointRef)endpoint;

// tz addition for proper detection of midi device 
